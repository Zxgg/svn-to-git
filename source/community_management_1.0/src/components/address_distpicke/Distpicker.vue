...
export default {
  ...
  props: {
    ...
    code: { type: [String, Number], default: '' },//行政区划，主要是向外传递数据
  ...
  watch: {
    currentProvince (value) {
      this.$emit('update:province', value.length > 1 ? value : null)
      let result = this.setData(value, 'province')
      this.$emit('province', result)
      this.$emit('update:code', result.code)
      if (this.onlyProvince) this.emit('selected')
    },
    currentCity (value) {
      this.$emit('update:city', value.length > 1 ? value : null)
      let result = this.setData(value, 'city', this.currentProvince)
      this.$emit('city', result)
      this.$emit('update:code', result.code)
      if (value != this.placeholders.city && this.hideArea) this.emit('selected')
    },
    currentArea (value) {
      this.$emit('update:area', value.length > 1 ? value : null)
      let result = this.setData(value, 'area', this.currentProvince, true)
