  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading"
      ...
      <el-table-column label="处理进度"
        ...
        <template slot-scope="scope">
          <span>
            <el-progress :percentage="scope.row.eventStatus === '0' ? 0: parseInt(scope.row.eventStatus) / ((eventStatusOptionsLength - 1) / 100)"
                         :show-text="false"
      ...
      <el-table-column label="操作"
        ...
        <template slot-scope="scope">
          <el-button size="mini"
                     ...
                     v-hasPermi="['communityEvent:community_event:Acceptance']">受理</el-button>
          <el-button size="mini"
                     type="text"
                     icon="el-icon-edit"
          ...
          <el-button size="mini"
                     ...
                     v-hasPermi="['communityEvent:community_event:Disposal']">办结</el-button>
          <el-button size="mini"
                     type="text"
                     icon="el-icon-edit"
                     v-if="scope.row.eventStatus === '3'"
                     @click="handleReview(scope.row)"
                     v-hasPermi="['communityEvent:community_event:Review']">归档</el-button>
          <!-- <el-button type="text"
                     icon="el-icon-plus"
                     size="mini"
    ...
    <el-dialog :title="title"
      ...
      <el-row :gutter="20">
        <el-col :span="formSpan"
          ...
          <el-form ref="form"
            ...
            <el-card class="box-card"
                     v-if="evemtHandleShow || imageUploadShow">
              <el-row :gutter="20">
                <el-col :span="12">
                  <el-form-item :label="eventStatusLabel"
                                v-if="evemtHandleShow"
                                prop="handleById">
                    <el-select v-model="form.handleById"
                               filterable
                               clearable
                               :disabled="isHandle"
                ...
                <el-form :model="item"
                  ...
                  <el-row :gutter="20">
                    <el-form-item label="办结人"
                                  ...
                                  prop="handleBy"
                                  label-width="100px">
                      <el-input v-model="item.handleBy"
                                placeholder="办结人" />
                  ...
                  <el-row :gutter="20"
                    ...
                    <el-form-item :label="item.eventStatus == 3?'办结描述':'归档意见'"
                                  prop="content">
      ...
      <div slot="footer"
        ...
        <el-button type="primary"
                   @click="submitForm">{{buttonName}}</el-button>
...
import { listInfo, getInfo, softDelInfo, delInfo, addInfo, exportInfo, updateEventsStatus } from "@/api/communityEvent/info";
import { listRecord, addRecord, getRecord, updateRecord, getRecordInfo, updateInfo, listPersonChuzhi } from "@/api/communityEvent/record";
  ...
  methods: {
        ...
        if (valid) {
          if (self.form.id != null) {
            ...
            if (self.form.eventStatus == 2) {
              ...
              if (handleById != null) {
                let arr = self.HandleByList.filter(item => item.id == handleById);
      ...
      getRecordInfo(id).then(response => {
        ...
        if (self.form != undefined) {
          ...
          if (self.form.eventInfoFileId != null) {
            ...
            self.eventInfoFileId = ['https://fuss10.elemecdn.com/e/5d/4a731a90594a4af544c0c25941171jpeg.jpeg', 'https://fuss10.elemecdn.com/e/5d/4a731a90594a4af544c0c25941171jpeg.jpeg'];
          }
    ...
    handleAssignment (row) {
      ...
      getRecordInfo(id).then(response => {
        ...
        console.log(this.form.eventStatus, 'this.form.eventStatus');
        ...
        if (this.form != undefined) {
          ...
          // [{"name":"4.jpg","url":"/group1/M00/00/14/rBUPNmCWYmSAJr2gAABErrr-vdk630.jpg?token=a57f05f08c55fbc989b61f61d1094f19&ts=1620470468"}]
    ...
    handleReview (row) {
      ...
      getRecordInfo(id).then(response => {
        ...
        if (this.form != undefined) {
          ...
          if (this.form.eventInfoFileId != null) {
            this.form.fileId = this.form.eventInfoFileId
            let imgList = JSON.parse(this.form.eventInfoFileId)
            imgList.forEach(img => {
              this.eventInfoFileId.push(configInfo.imgEchoAddress + img.url)
            })
