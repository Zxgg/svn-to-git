...
<script>
import { listOrg_basic, getOrg_basic, delOrg_basic, addOrg_basic, updateOrg_basic, exportOrg_basic } from "@/api/communityManage/org_basic";
...
export default {
  ...
  created () {
    ...
    this.getList();
    this.getDicts("base_comprehensive_management_organization").then(response => {
  ...
  methods: {
    ...
    handleDelete (row) {
      ...
      this.$confirm('是否确认删除综治机构管理编号为"' + ids + '"的数据项?', "警告", {
