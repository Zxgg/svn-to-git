  ...
  <div class="app-container">
    <el-form :model="queryParams"
      ...
      <!-- <el-form-item label="职务" prop="position">
        <el-select v-model="queryParams.position" placeholder="请选择职务" clearable size="small">
          <el-option label="请选择字典生成" value="" />
        </el-select>
      </el-form-item>
      <el-form-item label="专业特长" prop="majorStrength">
        <el-select v-model="queryParams.majorStrength" placeholder="请选择专业特长" clearable size="small">
          <el-option label="请选择字典生成" value="" />
        </el-select>
      </el-form-item> -->
      ...
      <!-- <el-form-item label="固定号码" prop="tel">
        <el-input
      ...
      <el-form-item>
        ...
        <el-button type="primary"
                   ...
                   size="mini"
                   @click="handleAdd"
                   v-hasPermi="['communityManage:org_public_team:add']">新增</el-button>
        <el-button type="danger"
                   icon="el-icon-delete"
                   size="mini"
                   :disabled="multiple"
                   @click="handleDelete"
                   v-hasPermi="['communityManage:org_public_team:remove']">删除</el-button>
      </el-form-item>
    </el-form>
    ...
    <!-- <el-row :gutter="10"
      ...
      <el-col :span="1.5">
        <el-button type="success"
                   ...
                   :disabled="single"
                   @click="handleUpdate"
                   v-hasPermi="['communityManage:org_public_team:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger"
                   icon="el-icon-delete"
      ...
      <el-col :span="1.5">
        <el-button type="warning"
                   ...
                   size="mini"
                   @click="handleExport"
                   v-hasPermi="['communityManage:org_public_team:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading"
      ...
      <el-table-column label="手机号码"
                       align="center"
                       prop="phone" />
      <!-- <el-table-column label="固定号码" align="center" prop="tel" /> -->
      <!-- <el-table-column label="创建人" align="center" prop="userId" />
      <el-table-column label="创建单位" align="center" prop="deptId" />
      <el-table-column label="更新人" align="center" prop="uUserId" />
      <el-table-column label="更新单位" align="center" prop="uDeptId" /> -->
    ...
    <el-dialog :title="title"
      ...
      <el-form ref="form"
        ...
        <el-form-item label="专业特长"
                      prop="majorStrength">
          <el-select v-model="majorStrength"
                     multiple
                     clearable
                     placeholder="请选择专业特长">
            <el-option v-for="dict in majorStrengthOptions"
                       :key="dict.dictValue"
                       :label="dict.dictLabel"
                       :value="dict.dictValue"></el-option>
          </el-select>
...
<script>
import { listOrg_public_team, getOrg_public_team, delOrg_public_team, addOrg_public_team, updateOrg_public_team, exportOrg_public_team } from "@/api/communityManage/org_public_team";
import { listOrg_public, getOrg_public } from "@/api/communityManage/org_public";

export default {
  name: "Org_public_team",
  data () {
    return {
      // 遮罩层
      loading: true,
      ...
      rules: {
        orgName: [
          { required: true, message: "群防群治组织不能为空", trigger: "blur" }
        ],
        name: [
        ...
        idType: [
          { required: true, message: "证件代码不能为空", trigger: "change" }
        ],
        idNum: [
  ...
  created () {
    ...
    listOrg_public(param).then(response => {
  ...
  methods: {
    idNumChange (event) {
      if (this.form.idType && this.form.idType === '111') {
    ...
    getList () {
      this.loading = true;
      listOrg_public_team(this.queryParams).then(response => {
        this.org_public_teamList = response.rows;
    ...
    handleUpdate (row) {
      ...
      getOrg_public_team(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          if (this.form.id != null) {
            updateOrg_public_team(this.form).then(response => {
            ...
            addOrg_public_team(this.form).then(response => {
              if (response.code === 200) {
    ...
    handleDelete (row) {
      ...
      this.$confirm('是否确认删除群防群治队伍编号为"' + ids + '"的数据项?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function () {
        return delOrg_public_team(ids);
      }).then(() => {
        this.getList();
    ...
    handleExport () {
      ...
      this.$confirm('是否确认导出所有群防群治队伍数据项?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function () {
        return exportOrg_public_team(queryParams);
      }).then(response => {
