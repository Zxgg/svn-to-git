      ...
      <el-table-column label="操作"
        ...
        <template slot-scope="scope">
          <el-button type="text"
                     ...
                     v-hasPermi="['tool:gen:edit']">刷新表结构</el-button>
          <el-button type="text"
                     size="small"
                     icon="el-icon-view"
...
<script>
import { listTable, updateTable, previewTable, refreshTable, genCode, delTable } from "@/api/tool/gen";
  ...
  methods: {
    ...
    handleGenTable (row) {
      ...
      if (row.genType === "1") {
        ...
        downLoadZip("/tool/gen/batchGenCode?tables=" + tableNames, row.functionAuthor);
      }
    },
    /** 打开导入表弹窗 */
    openImportTable () {
    ...
    handleUpdateTable (row) {
      ...
      refreshTable(tableName).then(response => {
