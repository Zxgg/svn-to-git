    ...
    <el-form :model="queryParams"
      ...
      <el-form-item>
        ...
        <el-button type="primary"
                   ...
                   size="mini"
                   @click="handleAdd"
                   v-hasPermi="['imagesInformationLibrarys:imageVideoLibrary:add']">新增</el-button>
        <el-button type="danger"
                   icon="el-icon-delete"
                   size="mini"
                   :disabled="multiple"
                   @click="handleDelete"
                   v-hasPermi="['imagesInformationLibrarys:imageVideoLibrary:remove']">删除</el-button>
    ...
    <!-- <el-row :gutter="10"
      ...
      <el-col :span="1.5">
        <el-button type="success"
                   ...
                   :disabled="single"
                   @click="handleUpdate"
                   v-hasPermi="['imagesInformationLibrarys:imageVideoLibrary:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger"
                   icon="el-icon-delete"
      ...
      <el-col :span="1.5">
        <el-button type="warning"
                   ...
                   size="mini"
                   @click="handleExport"
                   v-hasPermi="['imagesInformationLibrarys:imageVideoLibrary:export']">导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch"
                     @queryTable="getList"></right-toolbar>
    </el-row> -->
    <el-table v-loading="loading"
              :data="imageVideoLibraryList"
...
<script>
import { listImageVideoLibrary, getImageVideoLibrary, delImageVideoLibrary, addImageVideoLibrary, updateImageVideoLibrary, exportImageVideoLibrary } from "@/api/imagesInformationLibrarys/imageVideoLibrary";
import uploadImage from "@/components/upload/uploadImage"
import configInfo from '@/webconfig/configInfo'
export default {
  name: "ImageVideoLibrary",
  ...
  methods: {
    ...
    getList () {
      this.loading = true;
      listImageVideoLibrary(this.queryParams).then(response => {
        this.imageVideoLibraryList = response.rows;
    ...
    handleUpdate (row) {
      ...
      getImageVideoLibrary(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateImageVideoLibrary(this.form).then(response => {
            ...
            addImageVideoLibrary(this.form).then(response => {
              if (response.code === 200) {
    ...
    handleDelete (row) {
      ...
      this.$confirm('是否确认删除视频信息库编号为"' + ids + '"的数据项?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function () {
        return delImageVideoLibrary(ids);
      }).then(() => {
        this.getList();
    ...
    handleExport () {
      ...
      this.$confirm('是否确认导出所有视频信息库数据项?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function () {
        return exportImageVideoLibrary(queryParams);
      }).then(response => {
