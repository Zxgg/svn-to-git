<template>
  <div class="app-container">
    <el-form :model="queryParams"
      ...
      <el-form-item>
        ...
        <el-button type="primary"
                   ...
                   size="mini"
                   @click="handleAdd"
                   v-hasPermi="['imagesInformationLibrarys:ImageVehicleinforLibrary:add']">新增</el-button>
        <el-button type="danger"
                   icon="el-icon-delete"
                   size="mini"
                   :disabled="multiple"
                   @click="handleDelete"
                   v-hasPermi="['imagesInformationLibrarys:ImageVehicleinforLibrary:remove']">删除</el-button>
    ...
    <!-- <el-row :gutter="10"
      ...
      <el-col :span="1.5">
        <el-button type="success"
                   ...
                   :disabled="single"
                   @click="handleUpdate"
                   v-hasPermi="['imagesInformationLibrarys:ImageVehicleinforLibrary:edit']">修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger"
                   icon="el-icon-delete"
      ...
      <el-col :span="1.5">
        <el-button type="warning"
                   ...
                   size="mini"
                   @click="handleExport"
                   v-hasPermi="['imagesInformationLibrarys:ImageVehicleinforLibrary:export']">导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading"
              :data="ImageVehicleinforLibraryList"
...
<script>
import { listImageVehicleinforLibrary, getImageVehicleinforLibrary, delImageVehicleinforLibrary, addImageVehicleinforLibrary, updateImageVehicleinforLibrary, exportImageVehicleinforLibrary } from "@/api/imagesInformationLibrarys/ImageVehicleinforLibrary";
import uploadImage from "@/components/upload/uploadImage"
import configInfo from '@/webconfig/configInfo'
export default {
  name: "ImageVehicleinforLibrary",
  ...
  methods: {
    ...
    getList () {
      this.loading = true;
      listImageVehicleinforLibrary(this.queryParams).then(response => {
        this.ImageVehicleinforLibraryList = response.rows;
    ...
    handleUpdate (row) {
      ...
      getImageVehicleinforLibrary(id).then(response => {
        this.form = response.data;
        this.setPicsInfo(this.form)
        this.open = true;
        this.title = "修改车辆图像信息库";
      });
    },
    /** 提交按钮 */
    submitForm () {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateImageVehicleinforLibrary(this.form).then(response => {
            ...
            addImageVehicleinforLibrary(this.form).then(response => {
              if (response.code === 200) {
    ...
    handleDelete (row) {
      ...
      this.$confirm('是否确认删除车辆图像信息库编号为"' + ids + '"的数据项?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function () {
        return delImageVehicleinforLibrary(ids);
      }).then(() => {
        this.getList();
    ...
    handleExport () {
      ...
      this.$confirm('是否确认导出所有车辆图像信息库数据项?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(function () {
        return exportImageVehicleinforLibrary(queryParams);
      }).then(response => {
