		...
		<view class="fanku_con">
			<view class="img_z">
				...
				<view style="position: relative;" v-for="(item,index) in img_list" :key="index">
					<image @click="preview(index,img_list)" style="width: 200rpx;height: 200rpx;
					margin-left: 20rpx;margin-bottom: 20rpx;border-radius: 10rpx;"
					 :src="item"></image>
						...
						res.tempFilePaths.forEach((item, index) => { 
							...
							uni.uploadFile({ //上传图片
								...
								success: res => {
									...
									let url = strToObj.data;
									console.log(url);
									that.$emit('obtain_img', url)
									that.img_list.push(url); //plus数组
