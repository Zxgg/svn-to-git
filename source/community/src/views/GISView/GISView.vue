...
import { listCommunity_event, getCommunity_event, baseEventInfoList, handleEvents } from "@/api/communityEvent/community_event";
...
export default {
  ...
  components: { areapop, featurepop, vehicletrack, abnormalEvent, videoPlay, topgrid },
  mixins: [initMap, initArea, cluster, getData, location, track, drawMap, measure, mapOverlay],
  filters: {},
  computed: {
  ...
  methods: {
    ...
    deviceLocation (val) {
      // console.log(val, 'val')
      let feature = this.cameraSource.getFeatures().find(p => p.getProperties().cameraName === val.address)
      this.location(feature)
      let starTime_T = moment(val.reportingTime).subtract(15, 'seconds').format('YYYY-MM-DDTHH:mm:ss')
      let endTime_T = moment(val.reportingTime).add(15, 'seconds').format('YYYY-MM-DDTHH:mm:ss')
    ...
    async playVideoHistoryFuction (cameraSN, startTime, endTime) {
      ...
      let hls = await this.commonFunction.getHlsHistoryStreamByCameraCodeForContorl(cameraSN, startTime, endTime)
    ...
    eventLocation (data) {
      ...
      let feature = this.cameraSource.getFeatures().find(p => p.getProperties().cameraName === data)
      this.location(feature)
    },
    refreshData () {
  ...
  mounted () {
    this.initWebSocket();
    this.refreshData()
    this.$Notice.$on('VehicleTrack', (vehicleList) => this.vehicleTrack(vehicleList))
    this.$Notice.$on('VehicleLocation', (data) => this.vehicleLocation(data))
    this.$Notice.$on('sendaddress', (val) => { this.deviceLocation(val) })
    this.$Notice.$on('communityGridCannel', (val) => {
      this.isReturn = false
      this.areaLayer1.getSource().clear()
      if (this.$route.name === 'peopleInfo') this.$Notice.$emit('communityGridReturn', '')
    })
