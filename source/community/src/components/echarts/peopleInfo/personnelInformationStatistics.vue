...
import { getPerStatistics } from '@/api/PerInformation/perStatistics'
...
export default {
  ...
  mounted () {
    this.sexData = this.testData.renyuan.sexData
    this.$nextTick(() => {
      this.initChart()
    })
    // this.getPerStatisticsData()
  ...
  methods: {
            ...
            normal: {
              label: {
                ...
                formatter: function (params) {
                  ...
                  var total = 0;
                  for (var i = 0; i < sexData.length; i++) {
                    total = sexData[i].value + total;
                  }
                  percent = ((params.value / (total == 0 ? 1 : total)) * 100).toFixed(0);
                  if (params.name !== '') {
                    return '{b| ' + params.name + '：} ' + '{c|' + percent + '%}' + '{d|人员数量：' + params.value + '}'
                  } else {
                    return '';
                  }
                },
    ...
    getPerStatisticsData () {
      ...
      getPerStatistics(communityId, communityChildId).then(res => {
        if (res && res.data.length > 0) {
          ...
          //   }
          //   this.sexData.push(xData)
          // });
          for (let index = 0; index < res.data.length; index++) {
            this.sexData[index].value = res.data[index].percent * 100
          }
        }
        this.$nextTick(() => {
          this.initChart()
