...
const location = {
  ...
  methods: {
    ...
    location (data) {
      ...
      let me = this
      me.lc_source.clear()
      me.cameraFeatures.forEach(vehicleFeature => {
        if (data.get('cameraName') === vehicleFeature.getProperties().cameraName) {
          // me.map.setView(new View({
          //   center: getCenter(vehicleFeature.getGeometry().getExtent()), // 格式：[x,y]
          //   // center: [119.41919061230607, 32.43940449673803], // 格式：[x,y]
          //   zoom: 16.5,
          //   minZoom: 16.5,
          //   maxZoom: 16.5,
          ...
          // }))
          me.locationFeature = new Feature({ geometry: new Point(vehicleFeature.getGeometry().getCoordinates()) })
          me.locationFeature.setProperties(vehicleFeature.getProperties())
          me.locationFeature.setProperties(data)
          me.lc_source.addFeature(me.locationFeature)
          me.flash()
          // let feature = me.cameraFeatures.find(p => p.getProperties().sn === data.sn)
          // me.vehiclePopup = this.popOverlay(this.vehicleElement)
          // me.map.addOverlay(me.vehiclePopup)
          // let coordinate = feature.getGeometry().getCoordinates()
          // me.vehiclePopup.setPosition(coordinate)
        }
      });
