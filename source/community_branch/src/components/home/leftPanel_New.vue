<template>
  <div class="Main" ref="MainView">
    ...
    <div class="notice" v-if="false" :style="noticeBackground">
    ...
    <div class="leftBox">
              ...
              >
                <div class="gridPersonRowDiv">
                  ...
                  <el-row class="gridPersonRowDivNumber">
                    <el-col :span="12">
                      <div class="gridPersonRowDivNumberNumber">
                        {{ data.permanentPer }}
                      </div>
      ...
      <div class="partyOrg">
        <el-col :span="24" class="car-boxTitle">
          <span>网格微家</span>
           <span>GRID MICRO HOME</span>
        </el-col>
        <el-col :span="24"
          ><div style="width: 94%; height: 100%" @click="fullVideo"></div
        ></el-col>
      </div>
      <el-row v-show="isFull" class="RealTimeVideoPlayView_VideoRow">
        <!-- <div
          class="RealTimeVideoPlayView_VideoDiv"
          id="RealTimeVideoPlayView_VideoDiv"
        >
          <div
            :style="styleStr"
            class="RealTimeVideoPlayView_VideoDiv"
            id="RealTimeVideoPlayView_VideoDiv_Border"
          ...
          >
            <videoPlay
              ...
              :videoPlayId=num
              style="height: 99.4%; width: 99.4%; object-fit: fill"
            ></videoPlay>
          </div>
        </div> -->
        ...
        >
          ...
          >
            <!-- <videoPlay :ref="'videoPlayer' + num"
                       :id="'OneScreenVideo'+ num"
                       @click.native="selectScreen($event)"
                       @isVideoPlay="getVideoPlayState()"
                       :videoPlayId=num
                       style="height:99.4%; width:99.4%; object-fit:fill;"></videoPlay> -->
            <videoPlay
              :ref="'videoPlayer' + num"
              :id="'OneScreenVideo' + num"
              :videoPlayId="num"
...
export default {
  ...
  components: {
    ...
    occupancyRateChart,
    houseNatureChart,
    liveTypeStatisticsChart,
    populationMobilityChart,
    gridHouseStatistical,
    realOrganizationChart,
    videoPlay,
  },
  data() {
    return {
  ...
  methods: {
    ...
    resizeContainer() {
      this.centContainerHeight =
        this.$refs.MainView.offsetHeight - 22 - 131 - 4 + "px";
    ...
    fullVideo() {
      ...
      let element = document.getElementById("RealTimeVideoPlayView_VideoDiv"); // 设置后就是   id==con_lf_top_div 的容器全屏
    ...
    setScreen(value) {
      ...
      this.screenColNum = Math.sqrt(this.screenNumber);
      // this.screenLength = 24 / this.screenColNum
      let heightAndWidth = (100 / this.screenColNum).toString();
      switch (this.screenColNum) {
        case 1:
          heightAndWidth = 100;
          break;
        case 2:
    ...
    playVideo() {
      // 获取配置的设备code。逗号隔开
      if (configInfo.gbCodeList) {
        let cameraCodes = configInfo.gbCodeList.split(",");
        for (let a = 0; a < cameraCodes.length; a++) {
          let nowSelectScreenId = 0;
          nowSelectScreenId = a + 1;
          this.playVideoFuction(cameraCodes[a], nowSelectScreenId);
        }
      }
    },
    // playVideo (cameraSN) {
    ...
    //   this.$refs.videoPlayer1[0].playVideo(hls)
    //   this.$refs.videoPlayer1[0].playVideo(hls)
    //   this.$refs.videoPlayer1[0].playVideo(hls)
    //   this.$refs.videoPlayer1[0].playVideo(hls)
    // },
    async playVideoFuction(cameraSN, SelectScreenId) {
      let hls = await this.commonFunction.getHlsStreamByCameraCode(cameraSN);
      if (hls) {
        if (SelectScreenId) {
          switch (SelectScreenId) {
            case 1:
              this.$refs.videoPlayer1[0].playVideo(hls);
              break;
            case 2:
              this.$refs.videoPlayer2[0].playVideo(hls);
              break;
            case 3:
              this.$refs.videoPlayer3[0].playVideo(hls);
              break;
            case 4:
              this.$refs.videoPlayer4[0].playVideo(hls);
              break;
          }
        }
      }
  ...
  mounted() {
    ...
    this.carePersonItem.push({
      name: this.testData.shouye.carePerson.oneNumTitle,
      value: this.testData.shouye.carePerson.oneNum,
    });
    this.carePersonItem.push({
      name: this.testData.shouye.carePerson.twoNumTitle,
      value: this.testData.shouye.carePerson.twoNum,
    });
    this.carePersonItem.push({
      name: this.testData.shouye.carePerson.threeNumTitle,
      value: this.testData.shouye.carePerson.threeNum,
    });
    this.carePersonItem.push({
      name: this.testData.shouye.carePerson.fourNumTitle,
      value: this.testData.shouye.carePerson.fourNum,
    });
...
.RealTimeVideoPlayView_VideoRow {
  height: 93.5%;
  margin-top: 8px;
}
.RealTimeVideoPlayView_VideoDiv {
