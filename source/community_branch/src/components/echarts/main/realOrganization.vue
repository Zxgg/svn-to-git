...
import { getCommunityOrg } from "@/api/homePage/communityOrg"
...
export default {
  ...
  methods: {
    initChart () {
      ...
      //symbolSize 散点气泡大小
      var symbolSizeData = [100, 90, 80, 80, 60];
      //
      //循环定义series的data值
      var datas = [];
    ...
    getCommunityOrgData () {
      ...
      // this.communityOrg = this.testData.shouye.renfangche.communityOrg
      let communityId = this.currentComunity.communityId
      let communityChildId = this.currentComunity.communityChildId
      getCommunityOrg(communityId, communityChildId).then((res) => {
        // console.log(res, '123545sadsa')
        if (res && res.data.length > 0) {
          for (let index = 0; index < res.data.length; index++) {
            this.communityOrg[index].value = res.data[index].num
          }
          ...
          //     value: data.num
          //   }
          //   this.communityOrg.push(community)
          // });
        }
        this.$nextTick(() => {
          this.initChart()
