...
export default {
  ...
  data () {
    return {
      ...
      upload: {
        ...
        // 上传的地址
        url: configInfo.DBRestAPIEndPointUri + "/system/user/importData",
      },
      // 查询参数
      queryParams: {
        pageNum: 1,
  ...
  mounted () {
    this.$nextTick(() => {
      ...
      this.upload.url = configInfo.DBRestAPIEndPointUri + "/system/user/importData"
    })
  },
  computed: {
    dataHeight () {
  ...
  methods: {
    ...
    handleUpdate (row) {
      ...
      getUser(id).then((response) => {
        ...
        self.postOptions = response.posts;
        self.roleOptions = response.roles;
        self.form.roleIds = response.roleIds;
        if (self.form.roleIds !== undefined && self.form.roleIds !== null && self.form.roleIds.length > 0) {
          for (let i = 0; i < self.form.roleIds.length; i++) {
            self.form.roleIds[i] = self.form.roleIds[i] + ''
          }
        }
        self.open = true;
        self.title = "修改用户";
    ...
    handleDelete (row) {
      ...
      const uNames = row.userName || this.userNames
      // this.$confirm( '是否确认删除用户编号为"' + userIds + '"的数据项?', "警告", {
      this.$confirm('是否确认删除用户名称为"' + uNames + '"的数据项?', "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      }
