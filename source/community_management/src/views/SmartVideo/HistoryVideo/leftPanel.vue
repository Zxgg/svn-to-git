...
import CamLens from "@/components/SmartVideo/HistoryVideo/CamLens";
...
export default {
  components: { cameraTree, PTZControl, CamLens },
  data () {
    return {
      orgList: [], // 组织机构列表
      orgIDsStr: "", // 组织机构字符串
    ...
    selectRemoteDeviceChange (value) {
      if (value && value !== null && value.length > 0) {
        ...
        if (list && list != null && list.length > 0) {
          ...
          if (
            this.searchCamera.uiStatus != null &&
            this.searchCamera.uiStatus === 1
          ) {
          ...
          getDeviceOrg(queryData).then((data) => {
            if (
              ...
              if (paraentOrgId === "-1" || paraentOrgId === "") {
                // 未分配
                for (let i = 1; i < data.data.length; i++) {
                  if (
                    data.data[i].orgId &&
                    data.data[i].orgId !== null &&
                    data.data[i].orgId !== "-1" &&
                    data.data[i].orgId !== ""
                  )
                    paraentOrgId = data.data[i].orgId;
                }
              }
              if (
                !(
              ...
              while (paraentOrgId !== "-1" && paraentOrgId !== "") {
                let organ = self.orgList.filter(
                  (item) => item.code === paraentOrgId
                );
              ...
              if (
                paraentOrgNames.lastIndexOf("/") ===
                paraentOrgNames.length - 1
              ) {
                paraentOrgNames = paraentOrgNames.substring(
                  0,
                  paraentOrgNames.length - 1
                );
              }
