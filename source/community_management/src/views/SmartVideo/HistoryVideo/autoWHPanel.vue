<template>
  <div class="Main"
    ...
    <!-- <el-tooltip class="item"
                  ...
                  content="录像列表"
                  placement="top-start">
        <span class="iconfont iconfontColor"
              style="font-weight:bold"
              @click="localRecordList()">&#xe6dc;</span>
      </el-tooltip> -->
    ...
    <el-drawer title="录像列表"
               :visible.sync="localRecordListDialogVisible"
               :with-header="false"
               ...
               size="40%"
               direction="rtl">
      <HistoryVideoDownLoad style="height:100%"
                            ref="screenShot">
      </HistoryVideoDownLoad>
    </el-drawer>
  </div>
</template>
...
import HistoryVideoDownLoad from '@/components/SmartVideo/HistoryVideo/HistoryVideoDownLoad'
import { searchVideoDownloadTask } from '@/api/smartVideo/VideoDowaLoad'

export default {
  components: { videoPlay, screenShot, HistoryVideoDownLoad },
  data () {
    return {
      that: this,
      disabledShot: false,
      ...
      localRecordListDialogVisible: false,
      cameraInfoData: {
        onlineCount: 0,
        taskType: null,
        code: '',
  ...
  methods: {
    ...
    localRecordList (tableData) {
      this.localRecordListDialogVisible = true
      if (tableData === null || tableData === undefined) {
        return
      }
      this.$nextTick(function () {
        this.$Notice.$emit('recordDialogVisible', tableData)
      })
    },
    selectScreen (self) {
    ...
    async commonVideoMethods (videoList) { // 多个摄像机播放历史视频
      this.localRecordListDialogVisible = false
      let self = this
      // if (videoList.length <= 4 && videoList.length > 1) {
      //   this.setScreen(4)
      // } else if (videoList.length <= 9 && videoList.length > 4) {
      //   this.setScreen(9)
      // }
      for (let i = 0; i < videoList.length; i++) {
        var hls = await self.commonFunction.getHlsHistoryStreamByCameraCode(videoList[i].cameraCode, videoList[i].startTime, videoList[i].endTime)
        console.log('历史回放：', hls)
        let videoNameID = 'videoPlayer' + (i + 1)
        self.$refs[videoNameID][0].playVideo(hls)
      }
      // for (let i = 0; i < this.cameraSnList.length; i++) {
      //   console.log(this.cameraSnList[i].sn)
      // }
      // for (var i = 0; i < self.cameraSnList.length; i++) {
      //   var hls = await self.commonFunction.getHlsHistoryStreamByCameraCode(self.cameraSnList[i].sn, startTime, endTime)
      //   // var hls = await this.commonFunction.getHlsStreamByCameraCode(this.cameraSnList[i].sn)

      //   let data = {
      //     cameraName: self.cameraSnList[i].name,
      //     cameraCodeData: self.cameraSnList[i].sn,
      //     strRtspData: hls,
      //     // nowSelectScreenIdData: 'OneScreenVideo' + (i + 1)
      //   }
      //   self.playFormsDataList.push(data) // 存储屏幕播放code 和流地址

      //   switch (i + '') {
      //     case '0':
      //       self.$refs.videoPlayer1[0].playVideo(hls)
      //       break
      //     case '1':
      //       self.$refs.videoPlayer2[0].playVideo(hls)
      //       break
      //     case '2':
      //       self.$refs.videoPlayer3[0].playVideo(hls)
      //       break
      //     case '3':
      //       self.$refs.videoPlayer4[0].playVideo(hls)
      //       break
      //   }
      // }
    }
  ...
  mounted () {
    ...
    this.$Notice.$on('playHistroyVideo', (videoList) => this.commonVideoMethods(videoList))
    this.$Notice.$on('recordtablegeted', (tableData) => this.localRecordList(tableData))
  }
}
</script>
