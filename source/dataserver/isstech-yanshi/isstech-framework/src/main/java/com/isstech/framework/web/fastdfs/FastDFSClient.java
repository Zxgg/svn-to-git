...
import com.isstech.common.utils.FileUtil;
...
public class FastDFSClient {
     ...
     * 根据指定的http路径上传文件
     *
     * @param httpFilepath 文件http路径
     * @return 返回上传成功后的文件路径
     */
    public String uploadFileWithHttpFilepath(String httpFilepath) throws FastDFSException {
        return uploadHttp(httpFilepath, null);
    }
     ...
     */
    public String uploadHttp(String httpFilepath, Map<String, String> descriptions) throws FastDFSException {
        ...
        try {
            ...
            //InputStream is = new URL(httpFilepath).openStream();
            // 获取文件名
    ...
    public String uploadImageSample(String imageUrl) {
        ...
        try {
            // 上传到服务器
            String filepath = uploadFileWithHttpFilepath(imageUrl);
     ...
     * @return 结果
     */
    public String uploadToHW(MultipartFile multipartFile, HttpServletRequest request) throws IOException {
        File file = null;
        String fileName = multipartFile.getOriginalFilename();
        if (multipartFile != null) {
            InputStream ins = null;
            ins = multipartFile.getInputStream();
            //解决 所选上传的文件，文件名称是中文，乱码的问题
            file = new File(FastDfsConfig.fileTempPath+multipartFile.getOriginalFilename());
            FileUtil.inputStreamToFile(ins, file);
            ins.close();
        }
        Map<String, String> formParams = new HashMap<>(2);
        formParams.put("fileName", file.getName());
        Resp resp = HttpClientUtil.doPostFile(FastDfsConfig.fileServerHWAddr, file, formParams);
        if(!"200".equals(resp.getStatus())){
            return null;
        }
        JSONArray date =  JSONArray.parseArray((String) resp.getData().toString());
        String casefileid = null;
        if (date != null && date.size() > 0) {
            for (int i = 0; i < date.size() ; i++) {
                JSONObject jsonObject = date.getJSONObject(i);
                JSONObject caseFile = jsonObject.getJSONObject("casefile");
                if(caseFile!=null){
                    casefileid = caseFile.getString("casefileid");
                }
            }
        }
        return casefileid;
