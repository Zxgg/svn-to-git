...
public class BaseEventInfoController extends BaseController {
    ...
    public TableDataInfo queryEventInfoList(BaseEventInfo baseEventInfo, HttpServletRequest request) {
        ...
        return getDataTable(list.stream()
                .sorted(Comparator.comparing(BaseEventInfo::getUpdateTime).reversed())
                .collect(Collectors.toList()));
    }
...
//    @PreAuthorize("@ss.hasPermi('isstechadmin:info:remove')")
     ...
     * @return
     */
    @PostMapping(value = "addEventReportManually")
    public AjaxResult addEventReportManually(@RequestBody BaseEventInfo baseEventInfo) {
        return baseEventInfoService.addEventReportManually(baseEventInfo);
    }
